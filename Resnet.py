...
class BottleneckBlock(nn.Module):
    def __init__(self,in_ch,out_ch, identity_downsample=None,stride=1):
        ...
        self.conv3 = nn.Conv2d(out_ch, out_ch*self.expansion, kernel_size=1, stride=1, padding=0)
...
class ResNet(nn.Module):  
    ...
    def _make_layers(self, block, num_residual_blocks, out_channels, stride):
        ...
        ''' So, only the first Bottle_neck layer is used to down sample the input (done by keeping the stride=2) after that stride is decreased to=1 making those layers to keep the dimension of input as same '''
